...
*/
const GRAMMAR = ((API_ENDPOINTS as unknown as { GRAMMAR?: GrammarEndpointMap }).GRAMMAR || {
...
function paginationPayload<T>(res: unknown) {
  ...
  return { data: ((inner as { data?: unknown })?.data || []) as T[], total: Number((inner as { total?: unknown })?.total || 0) };
}

export const grammarService = {
  paginationTopics: async (skip = 0, take = 20, where: GrammarTopicFilter = {}) => {
    const res = await apiService.post(GRAMMAR.TOPICS_PAGINATION, { skip, take, where });
  ...
  paginationStructures: async (skip = 0, take = 20, where: GrammarStructureFilter = {}) => {
    const res = await apiService.post(GRAMMAR.STRUCTURES_PAGINATION, { skip, take, where });
  ...
  updateMastery: async (grammarStructureId: string, isCorrect: boolean) => {
    const res = await apiService.post(GRAMMAR.ME_MASTERY, { grammarStructureId, isCorrect });
