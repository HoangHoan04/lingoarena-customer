...
import type { IeltsQuestion, IeltsSectionKey } from "@/types/ielts";
import { Flag, Layers, Zap } from "lucide-react";
import { useMemo, useState } from "react";

interface IeltsQuestionPaletteProps {
  questions: IeltsQuestion[];
  currentIndex: number;
  answers: Record<string, any>;
  flaggedIds: string[];
  activeSection: IeltsSectionKey;
...
export function IeltsQuestionPalette({
  questions,
  currentIndex,
  answers,
  flaggedIds,
  ...
  const groupedParts = useMemo(() => {
    const groups: { partNumber: number; partTitle: string; items: { q: IeltsQuestion; originalIdx: number }[] }[] = [];
    ...
    questions.forEach((q, originalIdx) => {
      ...
      if (!grp) {
        grp = {
          partNumber: partNum,
          partTitle: q.partTitle || (activeSection === "reading" ? `Passage ${partNum}` : activeSection === "writing" ? `Task ${partNum}` : `Part ${partNum}`),
          items: [],
        };
        groups.push(grp);
      }
    ...
    <div className="rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 p-3.5 shadow-2xs space-y-3">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5">
            <Layers className="size-3.5 text-rose-600 dark:text-rose-400" />
            <h3 className="text-[11px] font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">
              IELTS {activeSection.toUpperCase()} Navigator
            </h3>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-[10px] font-medium text-slate-400">
              <strong className="text-rose-600 dark:text-rose-400 font-bold">{answeredCount}</strong>/{total} ({progressPercent}%)
            </span>
            ...
            {onToggleFlag && (
              <button
                ...
                title={isCurrentFlagged ? "Bỏ cờ câu này" : "Gắn cờ câu này để xem lại"}
              >
                <Flag className={`size-3 ${isCurrentFlagged ? "fill-amber-400" : ""}`} />
              </button>
            )}
          </div>
        </div>
        ...
        <div className="h-1 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
          <div
            className="h-full rounded-full bg-linear-to-r from-rose-600 to-amber-500 transition-all duration-300"
      ...
      <div className="space-y-2.5 max-h-64 overflow-y-auto pr-0.5 custom-scrollbar">
        {groupedParts.map((group) => {
          ...
          const answeredInGroup = group.items.filter(({ q }) => isQuestionAnswered(q.id)).length;
          ...
          return (
            <div key={group.partNumber} className="space-y-1.5">
              <div className="flex items-center justify-between text-[9px] font-bold text-slate-400 uppercase tracking-wider px-0.5">
                <span className="truncate max-w-[150px]">{group.partTitle}</span>
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex items-center justify-center size-7 rounded-lg text-[10px] font-semibold transition-all cursor-pointer select-none ${
                        isCurrent
                          ? "bg-rose-600 text-white shadow-xs font-bold ring-1.5 ring-rose-400 z-10 scale-105"
                          : isAnswered
                      ...
                      title={`Câu ${originalIdx + 1}${isAnswered ? " (Đã làm)" : ""}`}
                    >
      ...
      {unansweredCount > 0 && (
        <div className="pt-1.5 border-t border-slate-100 dark:border-slate-800">
          <button
            ...
            className="w-full inline-flex items-center justify-center gap-1 py-1.5 px-2 rounded-lg bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-750 text-[10px] font-bold text-rose-600 dark:text-rose-400 transition-colors cursor-pointer"
          >
