...
import type { VstepQuestion, VstepSectionKey } from "@/types/vstep";
import { Flag, Layers, Zap } from "lucide-react";
import { useMemo, useState } from "react";

interface VstepQuestionPaletteProps {
  questions: VstepQuestion[];
  currentIndex: number;
  answers: Record<string, any>;
  flaggedIds: string[];
  activeSection: VstepSectionKey;
...
export function VstepQuestionPalette({
  questions,
  currentIndex,
  answers,
  flaggedIds,
  ...
  const groupedParts = useMemo(() => {
    const groups: { partNumber: number; partTitle: string; items: { q: VstepQuestion; originalIdx: number }[] }[] = [];
    ...
    questions.forEach((q, originalIdx) => {
      ...
      if (!grp) {
        grp = {
          partNumber: partNum,
          partTitle: q.partTitle || (activeSection === "reading" ? `Bài đọc ${partNum}` : activeSection === "writing" ? `Task ${partNum}` : `Part ${partNum}`),
          items: [],
        };
        groups.push(grp);
      }
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5">
            ...
            <h3 className="text-[11px] font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">
              VSTEP {activeSection.toUpperCase()}
            </h3>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-[10px] font-medium text-slate-400">
              <strong className="text-emerald-600 dark:text-emerald-400 font-bold">{answeredCount}</strong>/{total} ({progressPercent}%)
            </span>
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex items-center justify-center size-7 rounded-lg text-[10px] font-semibold transition-all cursor-pointer select-none ${
                        isCurrent
                          ? "bg-emerald-600 text-white shadow-xs font-bold ring-1.5 ring-emerald-400 z-10 scale-105"
                          : isAnswered
      ...
      {unansweredCount > 0 && (
        <div className="pt-1.5 border-t border-slate-100 dark:border-slate-800">
          <button
            ...
            className="w-full inline-flex items-center justify-center gap-1 py-1.5 px-2 rounded-lg bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-750 text-[10px] font-bold text-emerald-600 dark:text-emerald-400 transition-colors cursor-pointer"
          >
