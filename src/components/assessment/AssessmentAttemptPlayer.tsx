...
function flattenQuestions(
  ...
  const fromSections = (attempt?.attemptSections || []).flatMap(
    (section) => section.attemptQuestions || [],
  );
  const source = fromSections.length
    ? fromSections
    : attempt?.attemptQuestions || [];
  return [...source].sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
}

function hasPendingEssay(attempt?: AssessmentAttempt | null) {
  const questions = flattenQuestions(attempt);
  return (
    questions.some(
      (item) =>
        String(item.answer?.gradingStatus || "").toUpperCase() === "PENDING",
...
export default function AssessmentAttemptPlayer({
  ...
  const [timeLeft, setTimeLeft] = useState(() =>
    Math.max(
      0,
      Math.floor(
        (new Date(initialAttempt.expiresAt).getTime() - Date.now()) / 1000,
      ),
    ),
  );
  ...
  const answeredIds = useMemo(
    () =>
      new Set(
        questions
          .filter((item) => item.answer?.answerJson)
          .map((item) => item.id),
      ),
    [questions],
  );
  ...
  const handleAnswer = async (answerJson: PracticeAnswer) => {
    ...
    setSaving(true);
    try {
      await assessmentService.saveAnswer(attempt.id, {
        attemptQuestionId: current.id,
        answerJson,
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAttempt(result);
      const pending = hasPendingEssay(result);
      addToast(
        pending
          ? "Đã nộp bài — đang chờ chấm"
          : "Đã nộp bài và chấm trắc nghiệm",
        "success",
      );
    } catch (err: any) {
      ...
      <div className="mx-auto max-w-3xl px-4 py-12 text-center">
        <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8">
          <p className="text-sm text-slate-500">
            Đề này chưa có câu hỏi khả dụng.
          </p>
  ...
  if (isFinished) {
    ...
    const pending = hasPendingEssay(attempt);
    return (
      <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10 space-y-6">
        <div className="rounded-3xl bg-linear-to-r from-[#1b2950] via-brand to-[#1b2950] text-white p-8 shadow-2xl space-y-3">
          <div className="w-16 h-16 rounded-2xl bg-amber-400 text-white flex items-center justify-center shadow-xl">
            <Trophy className="size-8" />
          </div>
          <h1 className="text-2xl sm:text-3xl font-black">
            {pending ? "Đã nộp — chờ chấm" : "Kết quả bài làm"}
          </h1>
          <p className="text-sm text-slate-200">
            {pending
              ? "Phần tự luận đang chờ giáo viên hoặc tác vụ chấm. Chưa có band giả."
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <span
                  className={`text-xs font-black px-2.5 py-1 rounded-full ${
                    String(item.answer?.gradingStatus || "").toUpperCase() ===
                    "PENDING"
              ...
              {item.answer?.scoreAwarded != null && (
                <p className="mt-2 text-xs text-slate-500">
                  Điểm: {Number(item.answer.scoreAwarded)}
                </p>
              )}
            </div>
          ))}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6 sm:py-10 space-y-6">
      <div className="rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 p-4 flex flex-wrap items-center justify-between gap-3 shadow-md">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-brand dark:text-[#7b9bee]">
            {pickLocaleText(
              locale,
              attempt.assessment?.title,
              attempt.assessment?.titleEn,
            ) || "Bài đánh giá"}
          </p>
        ...
        <div className="lg:col-span-4 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-xl space-y-5 h-fit">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className="flex-1 bg-brand hover:bg-[#1e2f5e]"
            >
              Sau
            </Button>
          </div>
